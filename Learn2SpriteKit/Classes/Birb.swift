...
//  MOAI Demo
//
//  Created by Luigi Minniti on 23/02/22.
//
...
class Birb : SKSpriteNode, GameSprite, EventListenerNode {
    var initialSize: CGSize = CGSize (width: 250, height: 250)
    var textureAtlas: SKTextureAtlas = SKTextureAtlas(named: "birb")
    ...
    func update(_ currentTime: TimeInterval) {
        ...
        if let redSlimePhysicsBody = Birb().physicsBody {
            if redSlimePhysicsBody.velocity.dx <= 0.0 && redSlimePhysicsBody.velocity.dy <= 0.1{
                Birb().physicsBody?.velocity = CGVector(dx: 0, dy: 0)
                Birb().zRotation = 0
                }
            }
        }
...
//        self.physicsBody!.contactTestBitMask = PhysicsCategory.Enemy
    ...
    init (){
        ...
        createAnimation(frame1: "Birb2", frame2: "Birbone")
